...
// running on it.
func ServeHttp(containerInstanceArn *string,
	...
	server := setupServer(containerInstanceArn, dockerTaskEngine, cfg, logger)
	for {
		once := sync.Once{}
...
func setupServer(containerInstanceArn *string,
	taskEngine DockerStateResolver,
	cfg *config.Config,
	logger *mux.Logger) *http.Server {
	serverFunctions := map[string]func(w http.ResponseWriter, r *http.Request){
		...
		"/logs":        logsRequestHandlerMaker(logger),
	}
...
func logsRequestHandlerMaker(logger *mux.Logger) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		var responseJSON []byte
