...
func ServeHTTP(credentialsManager credentials.Manager, containerInstanceArn string, cfg *config.Config, state dockerstate.TaskEngineState) {
...
func setupServer(credentialsManager credentials.Manager, auditLogger audit.AuditLogger, state dockerstate.TaskEngineState, cluster string) *http.Server {
	...
	serverMux.HandleFunc(metadataPath, metadataV1Handler(state, cluster))
